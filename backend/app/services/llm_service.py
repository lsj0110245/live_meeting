...
class LLMService:
    ...
    async def correct_transcript(self, text: str) -> str:
        ...
        try:
            ...
            # 도메인 용어 사전 (음성 유사어 오인식 방지)
            domain_terms = """
            - 발음/기능 테스트: 가나다라마바사, 아자차카타파하, 마이크 테스트, 원형 테스트
            - 문장 부호: 마침표, 쉼표, 느낌표, 물음표, 문장 부호
            - 숫자/형식: 2026년 2월 11일, 010-1234-5678 (전화번호), 123-45-67890 (계좌번호)
            - IT 용어: API, Docker, FastAPI, STT, Accuracy, Latency, Robustness, 딥러닝, 전처리
            """

            prompt = f"""
            당신은 전문적인 STT(Speech-to-Text) 결과 교정 비서입니다. 
            다음 [원본] 텍스트의 오타와 음성 오인식(유사 발음 오류)을 수정하세요.

            **[교정 가이드라인]**
            1. **회의 문맥 유지**: 비즈니스 회의 문맥에 맞지 않는 단어는 유사한 발음의 올바른 비즈니스 용어로 교정하십시오. (예: "인정 보호" -> "문장 부호", "아자카타" -> "아자차카타파하")
            2. **숫자/기호 정규화**: 날짜, 시간, 전화번호, 계좌번호는 한국어 표준 표기법 또는 비즈니스 관례에 맞게 기호(-, :, .)를 사용하여 정규화하십시오.
            3. **자연스러운 연결**: 문장의 조사가 어색하거나 생략된 경우 자연스럽게 보충하십시오.
            4. **절대 금지**: 설명, 인사, 추가 조언, 사족(예: "수정했습니다", "없음")은 절대 출력하지 마십시오. 오직 교정된 순수 텍스트만 출력하세요.
            5. **도메인 사전 참고**: 다음 전문 용어들을 참조하십시오:
            {domain_terms}
            ...
            # [사원 제거 및 정제 로직 강화]
            # 1. 특정 패턴의 사족 제거 (LLM이 "교정 결과:" 와 같이 출력하는 경우 대비)
            corrected_text = re.sub(r'^(교정 결과|수정된 텍스트|결과|교정본):', '', corrected_text).strip()
            
            # 2. 사족 패턴 체크
            stop_words = ["제공해 주시면", "수정할 내용이", "어렵습니다", "정보가 필요", "출력하지 않습니다", "추가 정보", "원본 텍스트", "교정되었습니다"]
