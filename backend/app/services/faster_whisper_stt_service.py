    ...
    async def transcribe_realtime(self, audio_bytes: bytes, language: str = "ko", skip_duration_ms: int = 0) -> str:
        ...
        try:
            ...
            for segment in segments:
                ...
                # [추가] Whisper가 initial_prompt를 복사하는 환각 패턴 포함
                if re.search(
                    r"자막|박진희|vostfr|Subtitles|Thank you|시청해 주셔서|무단 전재|배포 금지|감사합니다"
                    r"|가나다라|아자차카|마바사|010-1234-5678|123-456-7890"
                    r"자연스러운 한국어 문장|자연스러운 한국어 문장으로|회의 녹음입니다"
                    r"|마이브 테스트|마이크\s*테스",
                    text, re.I
                ):
                    # 환각 패턴이 포함된 경우 스킵
                    print(f"[STT Filter] 환각 패턴 감지, 스킵: {text[:30]}")
                    continue
